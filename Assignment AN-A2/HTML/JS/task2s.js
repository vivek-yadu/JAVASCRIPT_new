...
let newProduct = productMaster.map((ele) => {
    if (ele.category == 'Soft Drink') {
        return { id: ele.id, name: ele.name, category: ele.category, price: ele.price + 3, quantity: ele.quantity }
    } else {
        return { id: ele.id, name: ele.name, category: ele.category, price: ele.price - 2, quantity: ele.quantity }
    }
